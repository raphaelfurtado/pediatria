...
<div x-data="{ open: false }" @keydown.escape.window="open = false" class="relative">
    ...
    <!-- Mobile Menu Drawer (Outside header to avoid clip by backdrop-filter) -->
    <div x-show="open" 
        ...
        <div class="fixed inset-y-0 right-0 w-full max-w-[320px] bg-white shadow-2xl flex flex-col h-full">
            <div class="px-6 py-6 border-b border-slate-50 flex items-center justify-between bg-white">
                <span class="font-heading font-extrabold text-2xl text-secondary uppercase">Menu Principal</span>
                        ...
                        <div x-data="{ subOpen: false }" class="border-b border-slate-50 last:border-0">
                            ...
                            @if($item->children->count() > 0)
                                <div x-show="subOpen"
                                    ...
                                    @foreach($item->children->where('is_active', true) as $child)
                                        <a href="{{ $child->url ?: '#' }}"
                                            class="block py-3 text-sm text-slate-600 font-bold hover:text-primary transition-colors">
                                            {{ $child->label }}
                                        </a>
                                    @endforeach
                                </div>
