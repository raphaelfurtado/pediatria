@php
    ...
    $waUrl = null;
    if ($wa) {
        if (\Illuminate\Support\Str::startsWith($wa, ['http://', 'https://'])) {
            $waUrl = $wa;
        } else {
            $digits = preg_replace('/\D/', '', $wa);
            // Adiciona o código do Brasil (55) se o número vier só com DDD + número.
            if (strlen($digits) <= 11) {
                $digits = '55'.$digits;
            }
...
@if($waUrl)
    <a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer"
        ...
        class="fixed bottom-6 right-6 z-[70] w-14 h-14 rounded-full bg-[#25D366] text-white shadow-2xl ring-2 ring-white flex items-center justify-center hover:scale-110 transition-transform print:hidden">
